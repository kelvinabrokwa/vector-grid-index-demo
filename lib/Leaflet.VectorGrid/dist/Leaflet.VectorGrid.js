...
// 			console.log('Vector tile water:', json.layers.water);	// Instance of VectorTileLayer
			...
			for (var layerName in json.layers) {
				...
				for (var i=0; i<json.layers[layerName].length; i++) {
					...
					feats.push(feat);
          var featHash = [layerName, feat.id, data.x, data.y, data.z].join(':');
          if (!(featHash in loadedFeatures)) {
            var geom = feat.toGeoJSON(coords.x, coords.y, coords.z).geometry;
